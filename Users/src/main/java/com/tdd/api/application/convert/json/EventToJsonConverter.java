...
public final class EventToJsonConverter implements DomainEventConverter<JsonNode, CourseEvent>{

	
	private ObjectMapper mapper;
	
	public EventToJsonConverter(ObjectMapper mapper) {
		this.mapper = mapper;
	}
	
	@Override
	public JsonNode convertSingle(CourseEvent event) {
		ObjectNode rootNode = mapper.createObjectNode();
		return convertSingleInternals(event, rootNode);
	}
	...
	@Override
	public JsonNode convertAll(List<CourseEvent> events) {
		// TODO Auto-generated method stub
		ObjectNode rootNode = mapper.createObjectNode();
		ArrayNode data = mapper.createArrayNode();
		if(events != null) {
			for(CourseEvent event: events) {
				data.add(convertSingle(event));
			}
